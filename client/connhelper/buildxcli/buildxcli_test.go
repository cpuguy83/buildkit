...
func setupTestBuilder(t *testing.T) (_ Spec, shutdown func()) {
	dir := t.TempDir()
	...
	healthpb.RegisterHealthServer(srv, &testHealthServer{healthpb.UnimplementedHealthServer{}})

	go func() {
		err := srv.Serve(l)
		if err != nil {
			t.Log("server exited with error:", err)
		}
	}()
	...
	cmd.Env = []string{"BUILDKIT_HOST=unix://" + l.Addr().String()}
